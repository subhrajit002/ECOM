...
export default function Product() {
  ...
  }, [dispatch, lavelThree, colorValue, sizeValue, priceValue, sortValue, pageNumber, stock]);
  ...
  const handleFilter = (value, sectionId) => {
    ...
    filterValues.length ? searchParams.set(sectionId, filterValues.join(",")) : searchParams.delete(sectionId);
                    ...
                    {sortOptions.map((option) => (
                      <MenuItem key={option.name}>
                        <button
                          ...
                          className={classNames('block px-4 py-2 text-sm', option.current ? 'font-medium text-gray-900' : 'text-gray-500')}
                        >
                          {option.name}
                        </button>
                      </MenuItem>
