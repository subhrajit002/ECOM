...
const SignIn = () => {
  ...
  useEffect(() => {
    if (error) {
      Swal.fire("Invalid Crediantials", "Please try again", "error")
    }
  }, [error])
  ...
  const handleSubmit = async (event) => {
    ...
    // Dispatch loginUser and handle the response
     dispatch(loginUser(userData)); // Wait for the dispatch to complete
